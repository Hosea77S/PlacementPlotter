...
STR_make_stringtab(enum table_alloc we_alloc, const char *comment,
    ...
    HTABLE *htab;
    int i;
    htab = (HTABLE *)FEF_malloc_def(sizeof(HTABLE),DATA_HTABLE);
    htab->size = INIT_TABLE_SIZE;
    htab->mask = htab->size-1;
    htab->limit = (3*htab->size)>>2;
    htab->has = 0;
    htab->table = (ENTRY *)FEF_calloc_def(1,sizeof(ENTRY)*htab->size,DATA_ENTRY);
	...
	{
    	tables[i] = htab;
	table_names[i] = (char *)FEF_malloc_def(strlen(comment)+1,DATA_STRING);
...
static void stringtab_grow(HTABLE *htab)
{
    ...
    htab->limit = (3*htab->size)>>2;
    htab->has = 0;
    htab->table = (ENTRY *)FEF_calloc_def(1,sizeof(ENTRY)*htab->size,DATA_HTABLE);
    ...
    FEF_free_def((char*)old_table,DATA_HTABLE);
    if (old_has != htab->has)
	    defrNerr(61);
}
...
static const char *stringtab_alloc_string(HTABLE *htab, const char *str)
{
    ...
    if (need > htab->nleft) {
	POOL *p = (POOL *)FEF_malloc_def(sizeof(POOL), "POOL");
...
void STR_destroy_stringtab(void *handle)
{
    ...
    if( i < MAX_TABLES )
        {
        tables[i] = 0;
        FEF_free_def(table_names[i],DATA_STRING);
        }
    /* now do the freeing of the table */
    FEF_free_def((char*)htab->table,DATA_ENTRY);
    /* free all the string blocks, if allocated */
    for(;htab->pool_base != NULL;) {
	POOL *next = htab->pool_base->next;
        FEF_free_def((char*)htab->pool_base, DATA_STRING);
	htab->pool_base = next;
	}
    FEF_free_def((char*)htab,DATA_HTABLE);
}
