...
double B0, B1, x, t0, t1, r, c, cap0, cap1;
  ...
  //#SOHEIL
  //This delay should be calculated based on delay model
  ...
  t0 = d0 + JS[0][0].max + (SPL_Delay*100);
  t1 = d1 + JS[1][0].max + (SPL_Delay*100);
  assert(equal(t0, t1));
  return(t0);
...
/***********************************************************************/
  //#SOHEIL
void printNodeInfo(NodeType* node_L, bool left)
{
/*   if (left)	
  cout << "\nLeft: "; 
  else cout << "\nRight: ";
  cout << node_L->id << " L,R,P=[" <<
  node_L->L << "," << node_L->R << "," << node_L->parent << "]\n";
  cout << "m_stnPt: " << node_L->m_stnPt.x << "," << node_L->m_stnPt.y << "\n"; */
  //cout << "ms-X: " << node_L->ms[0].xlow << "," << node_L->ms[0].xhi << "\n";
  //cout << "ms-Y: " << node_L->ms[0].ylow << "," << node_L->ms[0].yhi << "\n";
	
}

void printAreaInfo (AreaType* area, bool left)
{
/*  if (left) cout << "\nLeftArea:";
  else cout << "\nRightArea:";  
  cout << "0,0:" <<  area->line[0][0].x << "," <<
  area->line[0][0].y
  << " 0,1:" << area->line[0][1].x << "," <<
  area->line[0][1].y << "\n";
  cout << "1,0:" <<  area->line[1][0].x << "," <<
  area->line[1][0].y
  << " 1,1:" << area->line[1][1].x << "," <<
  area->line[1][1].y << "\n";
  
   std::cout << "MRs:\n" ;
  for (int i=0; i < area->n_mr; i++)
    std::cout << i << ": " << area->mr[i].x << "," << area->mr[i].y << "\n";	
  std::cout << "subtree cost: " << area->subtree_cost << "\n";	
  std::cout << "dist: " << area->dist << "\n";	
  std::cout << "L and R edgelens: "<< area->L_EdgeLen <<
  ", " << area->R_EdgeLen << "\n"; */
  
}

void printMSInfo (TrrType* ms)
{
  cout << "ms-X: " << ms->xlow << "," << ms->xhi << "\n";
  cout << "ms-Y: " << ms->ylow << "," << ms->yhi << "\n";
...
double tL, tR;
 ...
 //cout << "In calc_new_delay: Max = " << pt->max << " , Min = " << pt->min << endl;
...
int i;
  ...
  cout << "In set_delay_for_mss ("<< area->mr[i].min << 
  "," << area->mr[i].max << ")\n";
  }
...
/****************************************************************************/
/*  compute the merging cost (wire) of mr(i) with its possible neighbor mr(j), and */
...
double cost_inc, old_cost, cost;
  ...
  //cout << "merging cost is: " << cost << endl;
...
double t;
  ...
  assert(t > 0);    
  printf("\nshow_NNG_infomation(bst.cxx)\n\ncomparisons/nodes = %d/%d = %.1f \n", (int) t,n,  t/n);
...
double Tcost = 0, Tdist=0, t, cap; 
  ...
  printf("Total Delay: (calc_buffered_node_delay:bst_sub1.cxx) %f ps\n",calc_buffered_node_delay(v));
  if (R_buffer>0) {
...
TrrType old_L_MS,  old_R_MS; 
  ...
  //std::cout << "in calc_JS_sub2\n";		
  ldist = linedist(pt[0],pt[1],pt[2],pt[3], pts);
  //std::cout << "\n\tldist = " << ldist << " , area->dist= " << area->dist << "\n";
  if (equal(ldist,area->dist)) {
    //std::cout << "In equal(ldist,area->dist) if loop\n";	  
    old_JS[0][0] = JS[0][0];
    old_JS[0][1] = JS[0][1];
    old_JS[1][0] = JS[1][0];
    old_JS[1][1] = JS[1][1];
    ...
    //std::cout << "In else if (ldist < area->dist ) loop\n";	  
    area->dist = ldist;
    update_JS(area, pt, pts);
  }
  if (calc_line_type(JS[0][0],JS[0][1]) == MANHATTAN_ARC ) {
...
/*     for (int i=0; i <=1 ; i++)
    ...
    if (v1<0 && v2 <0) {
      //std::cout << "\nBefore: in calc_JS_sub1\n";		
	  //printAreaInfo(area, true);	
...
/*      if (n1 > 1 || n2 > 1){
		...
		std::cout << "Changing JSs:\n";
		for (int i=0; i <=1 ; i++)
		  for (int j=0; j<=1; j++)
			std::cout << i << "," << j << "=[" << JS[i][j].x << 
			  "," << JS[i][j].y << "]\n";		
	} 	  
 */	  
    } else {
  ...
  //cout << "Area_L : " << area_L->n_mr << " Area_R: " << area_R->n_mr << endl;
...
NodeType::Merge2Nodes( NodeType *node_L,NodeType *node_R) {
  ...
  printNodeInfo(node_L, true);
  printNodeInfo(node_R, false);
  
  //#SOHEIL
  //cout << "In Merge2Nodes\nL & R ids: " << node_L->id << ","
  //<< node_R->id << " lines:";
  //printAreaInfo(area_L, true);
  //printAreaInfo(area_R, false);
...
void intersectJS_ROWs
  (AreaType *area, AreaType *area_L,AreaType *area_R)
{
...
/****************************************************************************/
/*    This only calculates the (wire) mergin cost, doesn't consider splitter delay mergin cost                                                                      */
/****************************************************************************/
...
AreaType area, *area_L, *area_R;
  ...
  double mergingCost = area.subtree_cost;
  //#SOHEIL
  return(mergingCost);
}
/****************************************************************************/
...
/*    std::cout << "IN if loop: Manhattan_arc\n";	  
	std::cout << "n_mr: " << area->n_mr << "\n";
	for (int i=0; i < area->n_mr; i++)
      std::cout << i << ": " << area->mr[i].x << "," << area->mr[i].y << "\n";	
	std::cout << "n_mr_area_R: " << area->area_R->n_mr << "\n";
	for (int i=0; i < area->area_R->n_mr; i++)
      std::cout << i << ": " << area->area_R->mr[i].x << "," << area->area_R->mr[i].y << "\n";	

	std::cout << "n_mr_area_L: " << area->area_L->n_mr << "\n";
	for (int i=0; i < area->area_L->n_mr; i++)
      std::cout << i << ": " << area->area_L->mr[i].x << "," << area->area_L->mr[i].y << "\n";	

    std::cout << "area->line\n";
	std::cout << "0,0: " << area->line[0][0].x << "," << area->line[0][0].y << "\n";
	std::cout << "0,1: " << area->line[0][1].x << "," << area->line[0][1].y << "\n";
	std::cout << "1,0: " << area->line[1][0].x << "," << area->line[1][0].y << "\n";
	std::cout << "1,1: " << area->line[1][1].x << "," << area->line[1][1].y << "\n";
...
/*   std::cout << "\nAfter Merge2Nodes:\n";  
  printNodeInfo(&Node[v], true );
  print_node(&Node[v]);
  print_node_info(&Node[v]);
  printAreaInfo(Node[v].area, true);   */
  //printMSInfo(&Node[v].area->L_MS);
  //printMSInfo(&Node[v].area->R_MS);

/*   if (Node[L].area->n_mr>1 || Node[R].area->n_mr>1){
	if (Node[L].area->n_mr>1){
		Node[L].area->mr[0].y += 1.11; 
		Node[L].area->line[0][0].y += 1.11;  
		Node[L].area->line[0][1].y += 1.11;  
		//Node[L].area->mr[1].y -= 1.11; 		
	    //Node[v].area->L_EdgeLen -= 1.11;
	}  
	
	if (Node[R].area->n_mr>1){
		Node[R].area->mr[0].y += 1.11;  
		Node[R].area->line[0][0].y += 1.11;  
		Node[R].area->line[0][1].y += 1.11;  
		//Node[R].area->mr[1].y -= 1.11; 		
		//Node[v].area->R_EdgeLen -= 1.11;
	}  
	
	Node[v].area->dist = Point_dist(Node[L].area->mr[0], Node[R].area->mr[0]) ; 
	//calc_JS_delay(Node[v].area, Node[L].area, Node[R].area);
  
	
  std::cout << "\n\tAfter changing mr by 1.11:\n";  
  printAreaInfo(Node[v].area, true);  
  } */
...
int i, j, k, n;
  ...
  while (n_trees > 1) {
    ...
    for (k=0;k<n;k++,v++) {
	  ...
	  //#SOHEIL
	  //for (int nmr=0; nmr < Node[v].area->n_mr; nmr++){
		//  Node[v].area->mr[nmr].max += SPL_Delay;
		//  Node[v].area->mr[nmr].min += SPL_Delay;		  
	  //}
	  
	  //cout << "ExG_DME Node(" << v << "(" << Node[v].area->n_mr  << ")) max(" <<
	  //Node[v].area->mr[0].max << ") min(" << Node[v].area->mr[0].min << ")\n"; 
...
void init() {
  ...
  int nterms = (int) gBoundedSkewTree->Nterms() ;
  cout << "nterms is : " << nterms << endl;
  
  N_Clusters[0] = nterms;
  //cout << "N_Clusters[1] is : " << N_Clusters[1] << endl;
  if (N_Clusters[1]<=0) {
...
int i, j;
  for (i=0;i<Node[v].n_area;++i) {
    for (j=0;j<Node[v].area[i].n_mr; ++j) {
	  ...
	  //cout << "set_MaxClusterDelay_sub: " << MaxClusterDelay << endl;
    }
  }
}
/**********************************************************************/
...
void add_buffer (int L, int n_buffer, int buffered_Node[]) {
  ...
  cout << "\nTotal number of buffered_Node: " << n_buffer << endl;
...
BST_DME::BST_DME (
    ...
    DelayModelType delayModel
     ...
     m_bstdme = new BstTree (inputSinksFileName,inputTopologyFileName,
          inputObstructionFileName,skewBound, splD, delayModel );
} ;
/**********************************************************************/
/* Copyright (C) 1994-2000  by Andrew B. Kahng, C.-W. Albert Tsao    */
/**********************************************************************/
...
int getHeight(NodeType& root) {
  cout << root.id << endl;
  if (root.id == -1 ) return 0;
  else return max ( getHeight (Node[root.L]), getHeight(Node[root.R]) )+ 1;
}
...
void print_heights(){
  ...
  cout << i << ": H: " << getHeight(Node[i]) << endl; 
  }
...
BstTree::ConstructTree( ) {
  ...
  Skew_B = SkewBound () ; 
  SPL_Delay = getSplDelay();
  
  cout << "SPL_Delay: " << SPL_Delay << endl;
