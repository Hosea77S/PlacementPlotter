...
#define NIL    -1
#define BUCKET_CAPACITY   250000  /* max. number of elements in a bucket */

#define MAX_BUFFER_LEVEL   100

#define MANHATTAN         0
#define EUCLIDEAN         1

#define MAX_N_SKEW_SAMPLES 50000

#define N_BUFFER_SIZE 50

//#define MAX_N_SINKS       3101
#define MAX_N_SINKS       500000
...
public:
  ...
  void SetTopologyFileName (const std::string &fn ) {
  ...
  void SetObstructionFileName (const std::string &fn ) {
  ...
  std::string SinksFileName () const {
  ...
  std::string ObstructionFileName () const {
...
private:
// data:
   std::string m_inputSinksFileName ;
   std::string m_inputTopologyFileName ; 
   std::string m_inputObstructionFileName ;

   double  m_skewBound ;
   double  m_splDelay  ;
   BST_DME::DelayModelType   m_delayModel ;
   
   std::vector <NodeType *> m_treeNode ;
   std::vector <NodeType *> m_candiRoot ;
   std::vector <NodeType *> m_tempNode ;
