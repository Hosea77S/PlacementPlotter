...
int type;
  ...
  if (type == MANHATTAN_ARC) {
	...
	//#SOHEIL //check here, may need to add spl delay
/* 	if (area->n_mr > 2){
        std::cout << "\nAdding to pt delay, q1:(" << q1->max <<
		"," << q1->min << "\n";
		q1->max += SPL_Delay;
		q1->min += SPL_Delay;  		
        std::cout << "After, q1:(" << q1->max <<
		"," << q1->min << "\n";
	} */
...
double d1,d2,a,b, A,B;
  ...
  //q1->max += SPL_Delay;
  //q1->min += SPL_Delay;
  //cout << "calc_pt_delays: Adding " << SPL_Delay << "\n";
...
double calc_node_delay(int v, double delay, int mode) {
  ...
  while (v!= root &&  Node[v].parent != NIL) {
	...
	//#SOHEIL
	//delay += SPL_Delay;
	cout << "calc_node_delay: Node: " << v << " delay = " <<
	delay << endl;
...
double t;
  ...
  if (Node[v].area[ca].n_mr==1) {
	 ...
	 //#SOHEIL
	 if (! equal(pt_skew(pt       ), pt_skew(Node[v].area[ca].mr[0])))
		 cout << "Mismatch: " << pt_skew(pt       ) << 
	     " != " << pt_skew(Node[v].area[ca].mr[0]) << endl;
...
void calc_BST_delay_sub(int v) {
  ...
  PointType &pt = Node[ v   ].m_stnPt ;
  //#SOHEIL 
  //most probably this is where should add spl delay
  pt.max =  tMAX( ptL     .max+tL, ptR      .max+tR);
  pt       .min =  tMIN( ptL     .min+tL, ptR      .min+tR);

  //#SOHEIL
  pt.max += (SPL_Delay*100);  
  pt.min += (SPL_Delay*100);    
...
/***********************************************************************/
/* Staring Soheil Methods*/
/***********************************************************************/

void print_bst_sub3_sol(FILE *f, int v) {
int p, n, sib;
double x,y,len,  dist, px, py, detour, off, xc,yc;
char pat;
PointType path[100];
  ...
  if (n>2) {
    ...
    assert(equal(y, qt       .y));
    assert( pat == 0 || pat == 1);
    fprintf(f,"\t%-3d\t%d\t%d\t%d\t%.2f\t%.2f\n",
	      v, Node[v].L, Node[v].R, p, x, y);
  }
  if (detour > 10000*FUZZ) {  /* detour */
    xc = (x+px)/2; 
    yc = (y+py)/2;
...
/***********************************************************************/
static 
void _print_bst_sub2_sol(FILE *f, int v) {

  if (v>=0) {
    if (Node[v].parent != NIL) {
      print_bst_sub3_sol(f,v);
    }
    if (Buffered[v]==NO) {
      _print_bst_sub2_sol(f,Node[v].L);
      _print_bst_sub2_sol(f,Node[v].R);
    }
  }
}
...
void print_root_sol(FILE *f){
	...
	PointType path[100];
    PointType &pt = Node[ parent   ].m_stnPt ;
    PointType &qt = Node[ root   ].m_stnPt ;
	dist = path_finder(qt, pt, path, &n);
	  ...
	  px = path[n-1].x;
	  py = path[n-1].y;
	//fprintf(f, "root id is : %d, parent id is : %d \n", root, parent); 
    //fprintf(f,"id:%-3d child:%d %d par:%d %.2f %.2f\n",
	//      parent, Node[parent].L, Node[parent].R, Node[parent].parent, x, y);
	fprintf(f, "NumNodes : %d\n", root+1); 
    fprintf(f,"\t%-3d\t%d\t%d\t%d\t%.2f\t%.2f\n",
	      root, Node[root].L, Node[root].R, Node[root].parent, px, py);
	
} 

static 
void print_bst_sub2_sol(FILE *f, int v) {
  print_root_sol(f);
  if (v>=0) {
    _print_bst_sub2_sol(f,Node[v].L);
    _print_bst_sub2_sol(f,Node[v].R);
...
/***********************************************************************/
static 
void print_bst_at_level_sol(FILE *out, int L) {
...
void write2ClocksFile(string outFilename)
{
	cout << "Writing to clocks file: " << outFilename << endl;

	FILE* file = fopen( outFilename.c_str(),"w");
	assert(file != NULL);
	
	
	fprintf(file, "#USC, generated by Soheil Nazar Shahsavani \n");
	fprintf(file, "#Format is as follows: \n");
	fprintf(file,"#ID\tLeftChild\tRightChild\tParent\tX_coordinate\tYcoordinate\n");
	//....
	//fprintf(file, "\n\n\n");
	
	  int n = calc_n_buffer_level();
	  for (int i=n-1;i>=0;--i) {
		print_bst_at_level_sol(file, i);
		fprintf(file, "\n\n");
	  }
	  //print_sinks_sol(file);
}


/***********************************************************************/
/* Ending Soheil Methods*/
/***********************************************************************/
...
double t;
  ...
  string clockFilename  = string ( fn).append(".clocks"); 
  cout << "open file (clock cells) " << clockFilename << endl << endl;
  write2ClocksFile(clockFilename);
